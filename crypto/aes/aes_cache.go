package aes

import (
	"bytes"
	"crypto/aes"
	"crypto/cipher"
	"encoding/base64"
	"errors"
	"fmt"
	"io/ioutil"
	"os"
	"path/filepath"
)
const DefaultCacheDirPath = "./.cache"

//加密过程：
//  1、处理数据，对数据进行填充，采用PKCS7（当密钥长度不够时，缺几位补几个几）的方式。
//  2、对数据进行加密，采用AES加密方法中CBC加密模式
//  3、对得到的加密数据，进行base64加密，得到字符串
// 解密过程相反

//16,24,32位字符串的话，分别对应AES-128，AES-192，AES-256 加密方法

//pkcs7Padding 填充
func pkcs7Padding(data []byte, blockSize int) []byte {
	//判断缺少几位长度。最少1，最多 blockSize
	padding := blockSize - len(data)%blockSize
	//补足位数。把切片[]byte{byte(padding)}复制padding个
	padText := bytes.Repeat([]byte{byte(padding)}, padding)
	return append(data, padText...)
}

//pkcs7UnPadding 填充的反向操作
func pkcs7UnPadding(data []byte) ([]byte, error) {
	length := len(data)
	if length == 0 {
		return nil, errors.New("加密字符串错误！")
	}
	//获取填充的个数
	unPadding := int(data[length-1])
	return data[:(length - unPadding)], nil
}

//AesEncrypt 加密
func AesEncrypt(data []byte, key []byte) ([]byte, error) {
	//创建加密实例
	block, err := aes.NewCipher(key)
	if err != nil {
		return nil, err
	}
	//判断加密快的大小
	blockSize := block.BlockSize()
	//填充
	encryptBytes := pkcs7Padding(data, blockSize)
	//初始化加密数据接收切片
	crypted := make([]byte, len(encryptBytes))
	//使用cbc加密模式
	blockMode := cipher.NewCBCEncrypter(block, key[:blockSize])
	//执行加密
	blockMode.CryptBlocks(crypted, encryptBytes)
	return crypted, nil
}

//AesDecrypt 解密
func AesDecrypt(data []byte, key []byte) ([]byte, error) {
	//创建实例
	block, err := aes.NewCipher(key)
	if err != nil {
		return nil, err
	}
	//获取块的大小
	blockSize := block.BlockSize()
	//使用cbc
	blockMode := cipher.NewCBCDecrypter(block, key[:blockSize])
	//初始化解密数据接收切片
	crypted := make([]byte, len(data))
	//执行解密
	blockMode.CryptBlocks(crypted, data)
	//去除填充
	crypted, err = pkcs7UnPadding(crypted)
	if err != nil {
		return nil, err
	}
	return crypted, nil
}

//EncryptByAes Aes加密 后 base64 再加
func EncryptByAes(data []byte, PwdKey []byte) (string, error) {
	res, err := AesEncrypt(data, PwdKey)
	if err != nil {
		return "", err
	}
	return base64.StdEncoding.EncodeToString(res), nil
}

//DecryptByAes Aes 解密
func DecryptByAes(data string, pwdKey []byte) ([]byte, error) {
	dataByte, err := base64.StdEncoding.DecodeString(data)
	if err != nil {
		return nil, err
	}
	return AesDecrypt(dataByte, pwdKey)
}

func EncryptByAesAndWriteToFile(pwdCachePath string, pwd string, addr string) error {
	//Cache password
	pwdStr, err := EncryptByAes([]byte(pwd), []byte(addr)[:32])

	path, err := cacheDir(pwdCachePath)
	if err != nil {
		return err
	}
	cachePath := fmt.Sprintf("%s/%s.pwd", path, addr)

	file, err := os.OpenFile(filepath.Clean(cachePath), os.O_EXCL|os.O_CREATE|os.O_WRONLY, 0600)
	if err != nil {
		return fmt.Errorf("unable to Open File: %w", err)
	}

	_, err = file.Write(append([]byte(pwdStr)))
	return err
}

func CheckPwdCacheExist(pwdCachePath string, addr string) bool {
	path, err := cacheDir(pwdCachePath)
	if err != nil {
		return false
	}
	cachePath := fmt.Sprintf("%s/%s.pwd", path, addr)

	// Make sure pwd exists before prompting password
	if _, err := os.Stat(cachePath); os.IsNotExist(err) {
		return false
	}

	fmt.Printf("pwd file found: %s\n", cachePath)
	return true
}

func GetPwdByReadCache(pwdCachePath string, addr string) (string, error) {
	path, err := cacheDir(pwdCachePath)
	if err != nil {
		return "", err
	}
	cachePath := fmt.Sprintf("%s/%s.pwd", path, addr)

	f, err := os.Open(cachePath)
	if err != nil {
		return "", fmt.Errorf("read file fail, %s", err)
	}

	pwd, err := ioutil.ReadAll(f)
	if err != nil {
		return "", fmt.Errorf("read to fd fail, %s", err)
	}
	return string(pwd), nil
}

// keystoreDir returns the absolute filepath of the keystore directory given a datadir
// by default, it is ./.cache
func cacheDir(path string) (cachePath string, err error) {
	if path != "" {
		cachePath, err = filepath.Abs(path)
		if err != nil {
			return "", err
		}
	} else {
		// path not specified, use default
		path = DefaultCacheDirPath

		cachePath, err = filepath.Abs(path)
		if err != nil {
			return "", fmt.Errorf("could not create pwd file path: %w", err)
		}
	}

	// if path does not exist, create it
	if _, err = os.Stat(path); os.IsNotExist(err) {
		err = os.Mkdir(path, os.ModePerm)
		if err != nil {
			return "", err
		}
	}

	// if path does not exist, create it
	if _, err = os.Stat(cachePath); os.IsNotExist(err) {
		err = os.Mkdir(cachePath, os.ModePerm)
		if err != nil {
			return "", err
		}
	}

	return cachePath, nil
}
